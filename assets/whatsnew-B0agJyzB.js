...
`;o.onReady(async()=>{const s=window.location.search,t=new URLSearchParams(s).get("subscriber")==="true";l.innerHTML=`
    ...
    `;const n=document.getElementById("closeButton");n.onclick=async()=>{await o.modal.close(i.EXTENSIONWHATSNEW)};const a=document.getElementById("discordButton");a.onclick=async e=>{e.preventDefault(),window.open("https://discord.gg/ANZKDmWzr6","_blank")};const r=document.getElementById("patreonButton");r.onclick=async e=>{e.preventDefault(),window.open("https://www.patreon.com/battlesystem","_blank")}});
